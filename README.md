...
- [`kicad-jlcpcb-tools`](https://github.com/bouni/kicad-jlcpcb-tools)
  installed — it maintains the `parts.db` SQLite catalog this tool
  reads from. Run its "Download parts data" step at least once.
...
Install `kicad-jlcpcb-tools` in KiCad and click "Download parts data"
inside the plugin. That populates the SQLite catalog at
`~/Library/Application Support/kicad/9.0/3rdparty/plugins/com_github_bouni_kicad-jlcpcb-tools/jlcpcb_parts.db`
(macOS) or the equivalent on your platform. `jlcpcb-mapper` auto-detects
this path; you can also point it elsewhere via `parts_db:` in the
config.
...
- [`kicad-jlcpcb-tools`](https://github.com/bouni/kicad-jlcpcb-tools)
  플러그인 설치 — 이게 `parts.db`를 만들어줍니다. KiCad에서 한 번
  "Download parts data" 실행해두세요.
...
KiCad에서 `kicad-jlcpcb-tools` 설치 후 플러그인의 "Download parts data" 한 번
실행. macOS 기준 SQLite 카탈로그가
`~/Library/Application Support/kicad/9.0/3rdparty/plugins/com_github_bouni_kicad-jlcpcb-tools/jlcpcb_parts.db`
에 생깁니다. `jlcpcb-mapper`가 자동 탐지하고, `parts_db:`로 직접 지정도
가능합니다.
